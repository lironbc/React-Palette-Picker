...
class ColorBox extends Component {
  ...
  render() {
    const copyClass = this.state.copied
      ? "copy-animation copied"
      : "copy-animation";

    /*check brightness of background to determine if text should be black/white for
          better contrast*/
    const isDarkColor = chroma(this.props.color).luminance() < 0.15;
    const isLightColor = chroma(this.props.color).luminance() >= 0.6;
    return (
      ...
      <CopyToClipboard text={this.props.color} onCopy={this.expandAnimation}>
        ...
        >
          ...
          <div className="box-content">
            <span className={isDarkColor ? "light-txt" : ""}>
              {this.props.name}
            </span>
          </div>
          ...
          <button
            className={
              isLightColor ? "copy-btn dark-txt dark-background" : "copy-btn"
            }
          >
            Copy
          </button>
          ...
          {this.props.activeMore && (
            ...
            >
              <div className="see-more">
                <span className={isLightColor ? "see-more dark-txt" : ""}>
                  More
                </span>
              </div>
            </Link>
